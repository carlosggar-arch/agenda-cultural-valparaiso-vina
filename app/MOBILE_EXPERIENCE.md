...
- `header-redesign.css` and `mobile-experience.css` are loaded from `app/index.html` inside `<head>` before first paint.
- JavaScript must not rewrite either existing stylesheet href after startup; doing so can trigger a second stylesheet load and visible layout movement.
- The header/mobile asset revisions used by `index.html`, `pwa.js`, the enhancement modules, and `service-worker.js` must stay synchronized.
...
- Static header controls must remain interactive after hydration; in particular the search button must be explicitly bound whether it came from HTML or was created dynamically.
