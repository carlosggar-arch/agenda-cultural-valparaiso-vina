...
// The renderer and the filters must consume the same normalized dataset.
...
// A release must actively check for a fresh service worker and the new worker must
// replace stale shell/data caches immediately. Existing controlled tabs then reload once.
