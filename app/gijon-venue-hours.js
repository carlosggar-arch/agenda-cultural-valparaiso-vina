import { hoursForDateFromDisplay } from "./date-aware-exhibition-hours.mjs?v=20260822-shared-hours2";
...
export function dateSpecificHours(display, dateKey) {
  return hoursForDateFromDisplay(display, dateKey);
}

export function gijonVenueHoursForDate(event, referenceDateKey) {
  const venue = registryHoursForEvent(event);
  if (!venue?.display) return null;
  const display = hoursForDateFromDisplay(venue.display, referenceDateKey);
