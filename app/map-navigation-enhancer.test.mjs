...
const accessibilitySource = readFileSync(new URL("./stage31-accessibility.css", import.meta.url), "utf8");
...
assert.match(accessibilitySource, /\.event-card a,[\s\S]*min-height:\s*44px/, "test fixture must cover the global event-card link touch rule");
assert.match(source, /width:25px !important/, "map arrow control width must win the global link cascade");
assert.match(source, /height:25px !important/, "map arrow control height must win the global link cascade");
assert.match(source, /min-width:25px !important/, "map arrow minimum width must remain exactly 25 px");
assert.match(source, /min-height:25px !important/, "map arrow minimum height must override the 44 px event-card link rule");
assert.match(source, /max-width:25px !important/, "map arrow maximum width must prevent inherited expansion");
assert.match(source, /max-height:25px !important/, "map arrow maximum height must prevent inherited expansion");
assert.match(source, /\.map-location-link::before\s*\{[\s\S]*width:44px;[\s\S]*height:44px;/, "the 25 px visual control must retain a 44 px touch target");
