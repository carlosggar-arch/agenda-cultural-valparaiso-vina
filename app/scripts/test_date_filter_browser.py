...
def card_tag(dom: str, event_id: str) -> str:
    match = re.search(
        rf'<article(?=[^>]*\bdata-event-id="{re.escape(event_id)}")[^>]*>',
        dom,
        flags=re.I,
    )
    if not match:
...
def card_html(dom: str, event_id: str) -> str:
    match = re.search(
        rf'(<article(?=[^>]*\bdata-event-id="{re.escape(event_id)}")[^>]*>.*?</article>)',
        dom,
        flags=re.I | re.S,
    )
    if not match:
...
def assert_selected_date(dom: str, selected: str) -> None:
    ...
    stale_tag = card_tag(dom, STALE_EVENT_ID)
    if not is_hidden(stale_tag):
