...
(async () => {
  ...
  if (!button) { document.body.dataset.favoritesProbe = 'missing-toggle'; return; }
  const id = button.closest('.event-card[data-event-id]')?.dataset.eventId || '';
  button.click();
  await sleep(700);
  let stored = [];
  ...
  document.body.dataset.favoritesProbe = saved && pressed && count === '1' && !document.querySelector('[data-my-plans]') ? 'pass' : 'fail';
})();
</script>
'''
