...
combined_filters_js = (APP / "combined-filters.js").read_text(encoding="utf-8")
combined_filters_css = (APP / "combined-filters.css").read_text(encoding="utf-8")
...
assert 'data-smart-search' in index
...
assert 'data-combined-price' in index
assert 'data-combined-category-filters' in index
assert 'data-date-from' in index and 'data-date-to' in index
...
assert 'function eventMatchesWhen' in combined_filters_js
assert 'function eventMatchesArea' in combined_filters_js
assert 'function eventMatchesPrice' in combined_filters_js
assert 'function eventMatchesCategories' in combined_filters_js
assert 'function eventMatchesQuery' in combined_filters_js
assert 'tokens.every((token) => haystack.includes(token))' in combined_filters_js
assert 'state.categories = new Set' in combined_filters_js
assert 'history.replaceState' in combined_filters_js
assert 'url.searchParams.set' in combined_filters_js
assert 'data-event-id' in combined_filters_js
assert 'forceBaseAppFilters' in combined_filters_js
assert 'currentCityId() !== "valparaiso"' in combined_filters_js
assert '.filter-workbench' in combined_filters_css
assert '.filter-grid' in combined_filters_css
assert '.custom-date-range' in combined_filters_css
assert '.smart-search' in combined_filters_css
assert '.legacy-filter-hooks{display:none!important}' in combined_filters_css
...
assert '"./combined-filters.css"' in shell_block
assert '"./combined-filters.js"' in shell_block
...
assert '"./sources-toggle.js"' in shell_block
