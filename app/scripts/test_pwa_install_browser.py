...
def make_test_page() -> None:
    ...
    bootstrap = (
        '<script>localStorage.setItem("agenda-cultural-city", "valparaiso");</script>\n  '
    )
    probe = r'''
  <script>
    (() => {
...
def main() -> None:
    ...
    with socketserver.TCPServer(("127.0.0.1", 0), handler) as server:
        ...
        time.sleep(0.2)
        try:
            with tempfile.TemporaryDirectory(prefix="agenda-installed-pwa-") as profile:
                cmd = [
                    chrome_binary(),
                    "--headless=new",
                    "--no-sandbox",
                ...
                if result.returncode != 0:
                    raise AssertionError(
                        f"Chrome failed during installed-PWA test: exit={result.returncode}\n"
                        f"STDERR:\n{result.stderr[-4000:]}"
                    )
                dom = result.stdout
        finally:
    ...
    required = {
        ...
        'data-pwa-controlled="true"': "Installed app is not controlled by its service worker after activation/reload",
        'data-pwa-version="PWA v24"': "Installed app did not load the current PWA v24 runtime",
