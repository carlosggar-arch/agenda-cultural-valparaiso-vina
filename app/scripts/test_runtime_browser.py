...
def run_city(city: str, base_url: str) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix=f"agenda-{city}-chrome-") as profile:
        ...
        if 'data-quick-filters="hoy,fin-de-semana,terminan-pronto,gratis,todos"' not in dom:
            raise AssertionError(f"Quick filters are not limited to time/free controls for {city}")
        ...
        print(f"Browser runtime {city}: {card_count} rich cards rendered with lean time/free controls")
