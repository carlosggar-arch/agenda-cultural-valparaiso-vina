...
def run_city(city: str, base_url: str) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix=f"agenda-{city}-chrome-") as profile:
        ...
        if f'data-active-city="{city}"' not in dom:
            raise AssertionError(f"Active city theme context did not apply for {city}")
        if 'data-visual-stripe="5px"' not in dom:
            raise AssertionError(f"Computed category card accent did not apply for {city}")
        for marker, message in (
            ('data-hero-title-display="none"', "Redundant hero title remained visible"),
            ('data-discovery-heading-display="none"', "Discovery explanation remained visible"),
            ('data-category-heading-display="none"', "Category explanation remained visible"),
            ('data-agenda-heading-display="none"', "Redundant result summary remained visible"),
            ('data-section-copy-display="none"', "Section explanatory copy remained visible"),
            ('data-search-compact="true"', "Search control is not compact"),
            ('data-category-no-wrap="true"', "Category controls do not stay compact"),
