...
def set_state(driver: webdriver.Chrome, state: str) -> None:
    selector = f'[data-section-filter="{state}"]'
    buttons = driver.find_elements(By.CSS_SELECTOR, selector)
    if not buttons:
        raise AssertionError(f"Missing canonical section filter: {state}")
    ...
    for city in city_ids():
        with tempfile.TemporaryDirectory(prefix=f"vivamos-parity-{name}-{city}-") as profile:
            ...
            try:
                ...
                url = f"{base}?city={city}&parity=online"
                driver.get(url)
