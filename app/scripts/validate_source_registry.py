...
def validate(registry: dict) -> dict:
    ...
    if not coverage_path.is_file():
        errors.append(f"missing_coverage_catalog:{coverage_path}")
        return {"status": "error", "errors": errors, "warnings": warnings}
    ...
    for city in coverage_by_city.values():
        for row in city.get("sources") or []:
            ...
            if sid:
                operational_ids.add(sid)
            if name:
                operational_names.add(name)
