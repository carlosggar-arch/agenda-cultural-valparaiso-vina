...
function addAlias(target, value, city) {
  const alias = clean(value).replace(/^[,·|/–—-]+|[,·|/–—-]+$/gu, "").trim();
  if (!alias) return;
  target.add(alias);
  if (!city) return;
...
function activityCategoryId(event) {
  return clean(event?.primary_category?.id || event?.categories?.[0]?.id).toLocaleLowerCase("es");
}
...
function smartAllCaps(value, event) {
  const category = activityCategoryId(event);
...
function normalizeBasePresentationTitle(value, event) {
  let text = stripRedundantFormatPrefix(value, event);
...
export function normalizeRootPublicEventTitle(value, event = null) {
  ...
  text = normalizeBasePresentationTitle(text, event);
