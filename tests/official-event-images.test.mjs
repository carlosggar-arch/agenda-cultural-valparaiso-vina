...
test("official event images survive the complete published dataset", async () => {
  ...
  for (const [title, url] of expected) {
    ...
    assert.equal(event.image?.origin_url || event.image?.url, url, `${title} must preserve its official provenance`);
    if (event.image?.url?.startsWith("./assets/event-images/")) {
      await access(new URL(`../app/${event.image.url.slice(2)}`, import.meta.url));
      assert.equal(event.image.cache?.source_url, url, `${title} cache provenance must match the official image`);
    }
